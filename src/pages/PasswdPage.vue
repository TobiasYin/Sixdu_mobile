<template>
  <div class="passwd-page">
    ...
    <div class="login">
      <div class="review">
        <!--</div>-->
        <mt-field label="旧的密码" placeholder="请输入密码" type="password" v-model="oldPassword"></mt-field>
        <mt-field label="新的密码" placeholder="请输入密码(大于六位)" type="password" v-model="password"></mt-field>
      ...
      <div class="button-outside">
        <mt-button type="primary" size="large" @click="passwd">修改密码</mt-button>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      passwd() {
        if (this.passwordState === 'success' && this.checkCodeState === 'success' && this.oldPassword.length !== 0){
          Indicator.open('提交中...');
          this.axios.post(this.url, {
            ...
            .then((resp) => {
              ...
              if (temp.success) {
                ...
                Toast('修改密码失败失败!原因:' + temp.failCode);
                this.getCheckCode();
              }
            })
            .catch((err) => {
