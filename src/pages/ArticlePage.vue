<template>
  <div class="page">
    <mt-header :title="articleData.title">
    ...
    <div class="title review"><span v-text="articleData.title"></span></div>
    <div class="author-time review">
      <router-link :to="'/user/'+articleData.authorID">
        <div class="author"><span v-text="articleData.author"></span></div>
      </router-link>
      <div class="time"><span v-text="articleData.time"></span></div>
    </div>
    <div class="review">
      <div class="content">
    ...
    <div class="comment">
      ...
      <ul>
        <li v-for="item in articleData.comment">
          <comment :ArticleItem="item"></comment>
        </li>
      </ul>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      clipBoardData() {
        return '六度，连接你我。\n' +
          '世界这么大，总有知你懂你之人。写下你的心声，寻找那个填补你心灵空缺的灵魂，愿我们都能拥有那奇迹般的瞬间！\n'+this.articleData.title+':'+this.$store.state.url + '/#' + this.$route.path
      },
      likeArticles() {
    ...
    methods: {
      clickLike() {
        if (this.isLiked === false) {
          this.axios.get(this.$store.state.url + '/api/like_article/' + this.$route.params.id, {timeout: 2500})
            .then(resp => {
              if (resp.data.success) {
                this.$store.commit('addUserLikeArticles', this.articleData.id);
          ...
          this.axios.get(this.$store.state.url + '/api/unlike_article/' + this.$route.params.id, {timeout: 2500})
            .then(resp => {
              if (resp.data.success) {
                for (let i = 0; i < this.likeArticles.length; i++) {
                  if (this.likeArticles[i] === this.articleData.id) {
                    this.isLiked = true;
      ...
      publishNewComment() {
        if (this.commentContent !== '') {
          MessageBox.confirm('确认发送？')
            .then(action => {
              Indicator.open('发送中...');
              this.axios.post(this.$store.state.url + '/api/new_comment/' + this.$route.params.id, {
